---
title: "Who Owns Ossium? Founder, Developer & Organization FAQ"
description: "Who founded ossium, who develops it, who owns the brand, and how the product is run-clear answers about Manish Kumar, MxCorpIn, and the team."
type: question
category: tools
tags: [ossium, founder, about, team]
keywords: [who owns ossium, ossium founder, who created ossium, manish kumar ossium, mxcorpin, ossium developer, who made ossium]
publishedAt: 2026-07-11
updatedAt: 2026-07-11
author: ossium
featured: true
image: /demo/oss_dashboardpage.webp
answerSummary: "Ossium was created and is primarily developed by Manish Kumar. The product at ossium.in is a proprietary service (not open source). Support: help@ossium.in."
---

People searching for **who owns ossium**, **who founded ossium**, or **who develops ossium** usually want a transparent answer-not corporate fog. Here’s the straightforward version.

## Short answer

| Question | Answer |
|----------|--------|
| Who founded / created ossium? | **Manish Kumar** |
| Who is the primary developer? | **Manish Kumar** (ongoing product & engineering) |
| Is ossium open source? | **No** - proprietary product |
| Product site | [ossium.in](https://ossium.in) |
| Support / contact | [help@ossium.in](mailto:help@ossium.in) · /contact |

## Who is the founder of ossium?

**Manish Kumar** is the founder and primary builder of ossium. Public signals include:

- Product author credit: **Manish Kumar**  
- Creator handle: **@Manixh02** on X (Twitter)  
- Product org handle: [**@ossium_inc**](https://x.com/ossium_inc)  
- Project organization context: **MxCorpIn**  

Ossium is an indie-built product: shipping speed, a clear mission to help open source *contributors*, and a sustainable optional Premium-not a giant corporate suite.

## Who owns ossium?

**Manish Kumar** founded and operates ossium. The live product at **[ossium.in](https://ossium.in)**-branding, infrastructure, Premium payments, support, and day-to-day operations-is run by the founder-led project.

Important clarification:

- **Product ownership:** founder-led proprietary product  
- **Not open source:** you use ossium as a service; the application is **not** released as open source software  
- **Mission:** help people discover and contribute to *other* open source projects  

See also: [What is ossium? FAQ](/what-is-ossium-faq) (includes “Is ossium open source? → No”).

## Who develops ossium today?

Day-to-day development is led by **Manish Kumar**. Feature ideas and feedback from the community are welcome through support channels (email, contact form, forum)-that is product feedback, not an open-source contribution model.

## What is MxCorpIn?

**MxCorpIn** is the organization / project context associated with ossium (including public GitHub presence where used). It is not a separate mystery megacorp. The human face of the product remains the founder and the public support channels.

## Is ossium a company, a nonprofit, or a side project?

Ossium is best described as a **founder-built product** with an optional paid Premium tier to keep the free core sustainable. It is **not** “Google’s ossium,” not an official GitHub product, and **not** an open-source foundation project.

## How is ossium funded?

Public model:

1. **Free core** for almost everyone (discovery, many tools, no login walls on core browse)  
2. **Optional Premium** at a very low price point (on the order of **₹17 / $0.19 per month**-see /premium)  
3. Community support channels (e.g. creator tip links where published)  

Premium is framed as supporting the product and unlocking extras-not locking essential contribution features.

## Who should I contact?

| Need | Channel |
|------|---------|
| Product support / questions | [help@ossium.in](mailto:help@ossium.in) |
| Contact form | /contact |
| Community discussion | /home/forum |
| Product updates | [x.com/ossium_inc](https://x.com/ossium_inc) |

## Can the community influence the roadmap?

Yes. Practical paths:

- Forum posts and discussion  
- Email to help@ossium.in  
- The /contact form  

The founder reviews suggestions and prioritizes impactful requests (also reflected on the landing FAQ).

## Is the founder still active?

Yes-ossium is under **active development** (2026). Features like dashboards, GSoC tools, alerts, forum, and Ossistant continue to evolve on the public site.

## Related FAQs

### Who created ossium?
**Manish Kumar.**

### Who is the CEO of ossium?
There isn’t a big-tech executive page. Treat it as a **founder-led** product; Manish Kumar is the person to associate with creation and direction.

### Is ossium owned by GitHub or Microsoft?
**No.** Ossium uses GitHub as a data source and workflow host for many open source projects, but it is an independent product.

### Is ossium open source?
**No.** Ossium is proprietary. It helps you find and track **open source** work; the platform itself is not open source.

### Where can I verify this myself?
- Product site: [ossium.in](https://ossium.in)  
- Product FAQ: [What is ossium?](/what-is-ossium-faq)  
- Premium / support: /premium, [help@ossium.in](mailto:help@ossium.in)

## Summary

**Ossium was founded and is primarily developed by Manish Kumar.** The official product is **[ossium.in](https://ossium.in)**. It is a **proprietary** service with a free core and optional Premium-not an open-source codebase. For trust and support, use the contact channels above-not third-party rumors.

Next reads:

- [What is ossium? Complete FAQ](/what-is-ossium-faq)  
- [Ossium vs GitHub Explore](/ossium-vs-github-explore)  
- [Best tools for open source contributors](/best-tools-for-open-source-contributors)  
